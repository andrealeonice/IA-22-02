...
class Server(object):
    ...
    def run(self) -> int:
        ...
        while True:  # runs until endgame
            ...
            # calculates scores
            p1_score = self.state.board.num_pieces(
                Board.BLACK)  # sum([1 for char in str(self.board) if char == self.board.BLACK])
            p2_score = self.state.board.num_pieces(
                Board.WHITE)  # sum([1 for char in str(self.board) if char == self.board.WHITE])
            ...
            if illegal_count[current_player] >= 5:
                print(
                    f'Player {current_player} ({self.player_dirs[current_player]}) DISQUALIFIED! Too many illegal move attempts or timeouts.')
                print('Game finished!')
